// Data-integrity test for the Decision NIA demo hub.
// Verifies the ANSWER_BANK + ASK_DATA structures inside
// DecisionNiaDemoHubClient.tsx have complete data for all 4 spaces.

import { test } from 'node:test';
import assert from 'node:assert/strict';
import { readFileSync } from 'node:fs';

const FILE = new URL('../components/pages/DecisionNiaDemoHubClient.tsx', import.meta.url).pathname.replace(/^\/([A-Z]:)/, '$1');
...
test('ASK_DATA has entries for all 4 spaces', () => {
  const block = blockBetween(SRC, 'const ASK_DATA:', 'const ANSWER_BANK');
  for (const space of SPACES) {
    assert.match(block, new RegExp(`(?:'${space}'|${space}):`), `ASK_DATA missing space ${space}`);
  }
});

test('ANSWER_BANK has extra Q+A entries for all 4 spaces', () => {
  const block = blockBetween(SRC, 'const ANSWER_BANK:', 'const FALLBACK');
  for (const space of SPACES) {
    assert.match(block, new RegExp(`(?:'${space}'|${space}):\\s*\\[`), `ANSWER_BANK missing space ${space}`);
  }
});
...
test('space picker popover is opaque (bg-black not transparent)', () => {
  ...
  assert.match(popoverMatch[0], /bg-black/, 'Popover must have bg-black to avoid transparent overlap');
  assert.doesNotMatch(popoverMatch[0], /bg-white\/\[0\.03\]/, 'Popover must not be transparent');
});

test('lookupAnswer + FALLBACK + ANSWER_BANK all present', () => {
